...
function EFFECT:Init(data)
	...
	self.Siyuz = 1
	self.DieTime = CurTime() + .1 * Scayul
	self.Opacity = 1
	self.TimeToDie = CurTime() + 0.01 * self.Scale
	...
	particle:SetDieTime(0.05 * Scayul)
	particle:SetStartAlpha(255)
	particle:SetEndAlpha(0)
	particle:SetStartSize(10 * Scayul)
	particle:SetEndSize(700 * Scayul)
	...
	for i = 0, 1 * Scayul do
		...
		particle:SetEndSize(math.Rand(500, 1000) * Scayul)
		particle:SetRoll(0)
	...
	timer.Simple(0.05, function()
		...
		for i = 0, 30 * Scayul do
			...
			local particle = Emitter:Add(sprite, vOffset)
			particle:SetVelocity(math.Rand(2000, 8000) * VectorRand() * Scayul + Vector(0, 0, math.Rand(0, 1000)) * Scayul)
			particle:SetAirResistance(1200)
			particle:SetGravity(VectorRand() * math.random(1, 2000))
			particle:SetDieTime(math.Rand(4, 7))
			particle:SetStartAlpha(math.Rand(50, 200))
			particle:SetEndAlpha(0)
			particle:SetStartSize(math.Rand(100, 140) * Scayul ^ .75)
			particle:SetEndSize(math.Rand(140, 280) * Scayul ^ .75)
...
function EFFECT:Think()
	if self.DieTime > CurTime() then
		self.Siyuz = self.Siyuz + 500 * self.Scayul
...
function EFFECT:Render()
	...
	local Opacity = math.Clamp(TimeLeftFraction * 4 * self.Scayul, 0, 255)
	...
	local TimeFraction = math.Clamp(TimeLeft / (0.01 * self.Scale), 0, 1)
