-- Jackarunda 2021
local Sprites={"sprites/mat_jack_smoke2"}--"particle/smokestack","particles/smokey","particle/particle_smokegrenade","sprites/mat_jack_smoke1","sprites/mat_jack_smoke2","sprites/mat_jack_smoke3"}
function EFFECT:Init(data)
    ...
    for i=1, 2 do
		local FireParticle=Emitter:Add(Firesprite..math.random(1,5),Pos)
		if FireParticle then
			FireParticle:SetVelocity(Vel+Norm*math.random(50, 100)+VectorRand()*10)
			FireParticle:SetAirResistance(5)
			FireParticle:SetDieTime(math.Rand(1, 3))
			FireParticle:SetStartAlpha(255)
			FireParticle:SetEndAlpha(0)

			local Size=math.Rand(30, 40)
			FireParticle:SetStartSize(Size/10)
	...
	for i=1, 2 do
		local RollParticle=Emitter:Add(Sprite,Pos+VectorRand()*2)
