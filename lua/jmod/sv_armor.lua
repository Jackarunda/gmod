...
function JMod_EZ_ToggleArmor(ply, slot)
    if not ply.EZarmor or not ply:Alive() or not slot then return end
    local id, data = GetArmorBySlot(ply.EZarmor.items, slot)
    if id == nil or not JMod_ArmorTable[data.name].tgl then return end
    ply:EmitSound("snds_jack_gmod/equip1.wav", 60, math.random(80, 120))
    data.disengaged = not data.disengaged

    if JMod_ArmorTable[data.name].eqsnd then
        ply:EmitSound(JMod_ArmorTable[data.name].eqsnd, 50, math.random(80, 120))
    end

    JModEZarmorSync(ply)
end

-- TODO generalize this and relevant functions
function JMod_EZ_Toggle_Mask(ply)
    if not ply.EZarmor or not ply:Alive() then return end
    local id, data = GetArmorBySlot(ply.EZarmor.items, "mouthnose")
...
function JMod_EZ_Toggle_Headset(ply)
    if not ply.EZarmor or not ply:Alive() then return end
    local id, data = GetArmorBySlot(ply.EZarmor.items, "ears")
    if id == nil then return end
