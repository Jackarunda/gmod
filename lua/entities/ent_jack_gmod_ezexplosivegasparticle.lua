-- Based off of JMOD EZ Gas Particle, explosive variant
AddCSLuaFile()
ENT.Base = "ent_jack_gmod_ezgasparticle"
ENT.PrintName = "EZ Explosive Gas"
ENT.Author = "Jackarunda, Adventure Boots"
...
if SERVER then
	...
	function ENT:Detonate()
		...
		timer.Simple(0.1, function()
			...
			-- Chain reaction: ignite nearby flammable entities
			for _, obj in pairs(ents.FindInSphere(SelfPos, SearchRadius)) do
				if obj ~= self and obj.JModHighlyFlammableFunc then
					local Func = obj[obj.JModHighlyFlammableFunc]
					if Func then Func(obj) end
				end
			end

			-- Create explosion (medium magnitude: 30-50)
			JMod.Sploom(JMod.GetEZowner(self), SelfPos, math.random(30, 50))
	...
	function ENT:OnTakeDamage(dmginfo)
		if dmginfo:IsExplosionDamage() or dmginfo:IsDamageType(DMG_BURN) or dmginfo:IsDamageType(DMG_SLOWBURN) then
			self:Detonate()
		end
	end
...
elseif CLIENT then
	...
	function ENT:Initialize()
		self.Col = Color(200, 160, 80) -- Yellowish/orange tint for explosive gas
		self.Visible = true
		self.Show = true
		self.siz = .5
	...
	function ENT:DrawTranslucent()
		...
		if self.DebugShow then
			...
			render.DrawSprite(self:GetPos(), 50, 50, Color(255, 160, 0, 200)) -- Orange debug color
		end
