...
AddCSLuaFile()
ENT.Type="anim"
ENT.PrintName="EZ Aid Teleporter"
...
ENT.EZconsumes={"antimatter","parts"}
...
if(SERVER)then
	...
	function ENT:TurnOff()
		...
		self:EmitSound("snd_jack_turretbatteryunload.wav",80,10)
	end
	...
	function ENT:TurnOn(activator)
		...
		self:EmitSound("snd_jack_genstart.mp3",85,30)
		self.ConnectionAttempts=0
	end
	function ENT:Connect(ply)
		...
		JMod_EZradioEstablish(self,StationKey,true)
	...
	function ENT:Think()
		...
		if(self.NextRealThink<Time)then
			...
			if(State==STATE_CONNECTING)then
				self:ConsumeElectricity(math.random(4,5))
			...
			timer.Simple(i/100,function()
				if(IsValid(self))then
					if(i<20)then
						...
						if(Powa>1)then
							local Batt=ents.Create("ent_jack_gmod_ezantimatter")
...
elseif(CLIENT)then
	function ENT:Initialize()
		self.Dish=JMod_MakeModel(self,"models/nseven/combine_intmonitor001.mdl","models/props_combine/combine_intmonitor001_disp_off")
	...
	local GlowSprite,StateMsgs=Material("sprites/mat_jack_basicglow"),{[STATE_CONNECTING]="Charging...",[STATE_READY]="Ready"}
	function ENT:Draw()
		...
		DishAng:RotateAroundAxis(Right,90)
		JMod_RenderModel(self.Dish,BasePos+Up*1+Forward*25,DishAng,nil,Vector(.7,.7,.7))
		---
		if(DetailDraw)then
			...
			if((Closeness<20000)and(State>0))then
				...
				cam.Start3D2D(SelfPos+Up*45-Forward*5,DisplayAng,.075)
				if(State>1)then
