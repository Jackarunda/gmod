...
AddCSLuaFile()
ENT.Type = "anim"
ENT.PrintName = "EZ Directional Shield Generator"
...
function ENT:CustomSetupDataTables()
	...
	self:NetworkVar("Float", 3, "ShieldDistance") -- Distance of shield from projector
end
...
if(SERVER)then
	function ENT:CustomInit()
		...
		-- Shield always faces backward toward generator (fixed direction)
		-- Shield will always appear at adjustable distance in front of generator
		self.ShieldDirection = -self:GetAngles():Right() -- Always faces back toward generator
		
		-- Default shield distance (adjustable)
		self:SetShieldDistance(200)
	...
	function ENT:Use(activator)
		...
		elseif State == STATE_OFF then
			if alt then
				-- Alt+Use to adjust shield distance
				self:AdjustShieldDistance(activator)
			else
	...
	function ENT:AdjustShieldDistance(activator)
		if not IsValid(activator) then return end
		
		local currentDistance = self:GetShieldDistance()
		local newDistance = math.Clamp(currentDistance + 50, 100, 550) -- Cycle through distances
		if newDistance >= 550 then newDistance = 100 end -- Reset to minimum
		
		self:SetShieldDistance(newDistance)
		
		self:EmitSound("buttons/button14.wav", 60, 120)
		JMod.Hint(activator, "Shield distance set to "..tostring(newDistance).." units")
		
		-- If shield is active, update its position
		if IsValid(self.Shield) then
			self.Shield.CustomShieldDistance = newDistance
			self.Shield:PositionShield()
		end
		
		-- Visual feedback - create beam to show shield position
		if CLIENT then return end
		local ShieldPos = self:GetPos() + self:GetAngles():Forward() * newDistance
		local BeamData = EffectData()
		BeamData:SetStart(self:GetPos())
		BeamData:SetOrigin(ShieldPos)
		BeamData:SetScale(1)
	...
	function ENT:EstablishShield()
		...
		-- Create directional shield
		self.Shield = ents.Create("ent_jack_gmod_directional_shield")
		self.Shield:SetPos(Pos)
		self.Shield.Projector = self
		self.Shield:SetSizeClass(self:GetGrade())

		-- Pass custom distance to shield entity (direction is fixed)
		self.Shield.ShieldDirection = self.ShieldDirection
		self.Shield.CustomShieldDistance = self:GetShieldDistance()
		...
		-- Note: Don't parent flat shields as they need to move independently
	end
	...
	function ENT:Think()
		...
		-- Update shield position if it exists (make shield follow generator movement)
		if IsValid(self.Shield) then
			self.Shield:FollowProjector()
		end
		...
		if (State == STATE_ON) then
			...
			if not (IsValid(self.Shield)) then
				...
				if (Accepted > 0) then
					...
					if (Accepted > 6) then -- high-power mode
		...
		self:NextThink(Time + .1) -- More frequent updates for better following
		return true
	end
...
elseif(CLIENT)then
	...
	function ENT:Draw()
		...
		if DetailDraw then
			if Closeness < 20000 and State > 0 then
				...
				local DistAmt = self:GetShieldDistance()
				...
				draw.SimpleTextOutlined("DISTANCE: "..tostring(math.Round(DistAmt)).."u", "JMod-Display", 190, 120, Color(255, 255, 0, Opacity), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, Opacity))
	...
	language.Add("ent_jack_gmod_ezdirectionalshieldgen", "EZ Directional Shield Generator")
end
