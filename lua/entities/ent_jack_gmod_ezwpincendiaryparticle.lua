...
ENT.PrintName = "EZ WP Cloud"
ENT.Author = "Jackarunda, AdventureBoots, Freaking Fission"
...
if SERVER then
	...
	function ENT:DamageObj(obj)
		...
		if obj:IsPlayer() then
			...
			if math.random(1, 2) == 1 and (faceProt + skinProt < 2) then
				local Dmg = DamageInfo()
				Dmg:SetDamageType(DMG_BURN + DMG_NERVEGAS)
	...
	function ENT:CalcMove(ThinkRateHz)
		...
		local Force = RandDir + (JMod.Wind * 2) + Vector(0, 0, -10)
		...
		if not MoveTrace.Hit then
			...
			local CurVelAng, Speed = self.CurVel:Angle(), self.CurVel:Length() * .3
...
elseif CLIENT then
	...
	function ENT:DrawTranslucent()
		...
		--if self.Show then
			self.Rotation = self.Rotation + FrameTime() * math.Rand(1, 10)
			local SelfPos = self:GetPos()
			render.SetMaterial(Mat)
			render.DrawQuadEasy(self.RenderPos, LocalPlayer():EyePos() - SelfPos, self.siz, self.siz, Color(self.Col.r, self.Col.g, self.Col.b, 150), self.Rotation)
			self.RenderPos = LerpVector(FrameTime() * .5, self.RenderPos, SelfPos)
