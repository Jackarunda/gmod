...
function ENT:InitPerfSpecs(removeAmmo)
	...
	for specName,value in pairs(self.DynamicPerfSpecs)do self[specName]=value*PerfMult*JMod.EZ_GRADE_BUFFS[Grade]^1.223 end
...
if(SERVER)then
	...
	function ENT:Use(activator)
		if(activator:IsPlayer())then
			...
			if(State==STATE_BROKEN)then JMod.Hint(activator, "destroyed") return end
			
			if(State>0)then
				self:TurnOff()
			else
				if (self:GetElectricity()>0) then self:TurnOn(activator) JMod.Hint(activator, "sentry friends")
